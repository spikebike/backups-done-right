...
func runBackupCycle(t *testing.T, clientDB *sql.DB, dbJobChan chan db.DBJob, rpcClient client.RPCClient, backupDirs []string, key []byte) int64 {
	...
	uploader := client.NewUploader((chan<- db.DBJob)(dbJobChan), uploadChan, rpcClient, 2, 10, false, false, backupID, nil)
...
func TestEndToEndBackup(t *testing.T) {
	...
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 10, 4, 10*1024*1024, true, nil, "", 1024, false, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
...
func TestLargeFile(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 2, 1, 100*1024*1024, true, nil, "", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
...
func TestIncrementalBackup(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 10, 4, 100*1024*1024, true, nil, "", 1024, false, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
...
func TestServerGC(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 10, 4, 5, true, nil, "", 1024, false, false, 8, 0, 0, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
	...
	ingestErr := engine.IngestItems(context.Background(), "system-self-backup", []rpc.ItemData{
		{Meta: rpc.Metadata{Hash: h, Size: int64(len("some data")), IsSpecial: true, Type: 0}, Data: []byte("some data")},
	}, false)
	if ingestErr != nil {
		t.Fatalf("IngestItems failed: %v", ingestErr)
	}
	var blobHash string
	serverDB.QueryRow("SELECT hash FROM blobs LIMIT 1").Scan(&blobHash)
	engine.DeleteItems(context.Background(), []string{blobHash})
	serverDB.Exec("UPDATE blobs SET deleted_at = datetime('now', '-1 hour'), ref_count = 0 WHERE hash = ?", blobHash)
	serverDB.Exec("UPDATE shards SET status = 'sealed'")
...
func (h *MockPeerHandler) OfferItems(ctx context.Context, call rpc.PeerNode_offerItems) error {
	args := call.Args()
	shards, _ := args.Items()
...
func (h *MockPeerHandler) ListSpecialItems(ctx context.Context, call rpc.PeerNode_listSpecialItems) error {
	var specials []rpc.Metadata
	...
	res, _ := call.AllocResults()
	shards, _ := res.NewItems(int32(len(specials)))
...
func TestOutboundWorkerFlow(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 1, 1, 10, true, nil, "", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
...
func TestChallengeWorkerFlow(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 1, 1, 1024, true, nil, "", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
	...
	mh.Pieces[pieceHash] = MockPiece{Data: data, Meta: rpc.Metadata{IsSpecial: false, Type: 1}}
...
func TestRepairWorkerFlow(t *testing.T) {
	...
	serverDB, _ := server.InitDB(serverDBPath)
	defer serverDB.Close()
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 2, 1, 1024*1024, true, nil, "", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "", 4, false, false, 0, 0)
	defer engine.Wait()
	...
	for _, e := range entries {
		if strings.Contains(e.Name(), "piece_2") {
			found = true
		}
	}
	if !found {
		t.Errorf("Piece 2 not enqueued for repair")
	}
}
...
func TestReedSolomonIntegration(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		handlers[i].Pieces[h] = MockPiece{Data: data, Meta: rpc.Metadata{IsSpecial: false, Type: 1}}
...
func TestDisasterRecovery(t *testing.T) {
	...
	hdr := &tar.Header{Name: "client.db", Size: int64(len(dbData)), Mode: 0644}
	tw.WriteHeader(hdr)
	tw.Write(dbData)
	tw.Close()
	enc.Close()

	cipher, _ := crypto.Encrypt(key, buf.Bytes())
	signature := crypto.SignRecoveryShard(key, cipher)
	signedCipher := make([]byte, 64+len(cipher))
	copy(signedCipher[:64], signature)
	copy(signedCipher[64:], cipher)

	h := hex.EncodeToString(crypto.Hash(signedCipher))
	hashBytes, _ := hex.DecodeString(h)
	item := rpc.StreamItem{
		Header: rpc.StreamItemHeader{
			OpCode: rpc.OpCodePush,
			Flags:  rpc.FlagTypeClientBlob,
			Size:   uint64(len(signedCipher)),
		},
		Data: bytes.NewReader(signedCipher),
	}
	copy(item.Header.Hash[:], hashBytes)

	_, err := rpcClient.OfferItems(context.Background(), []rpc.Metadata{
		{Hash: h, Size: int64(len(signedCipher)), IsSpecial: true, Type: 0},
	})
	if err != nil {
		t.Fatalf("Failed to prepare client bundle metadata: %v", err)
	}

	err = rpcClient.UploadItemsStreamed(context.Background(), []rpc.StreamItem{item})
	if err != nil {
		t.Fatalf("Failed to upload client bundle metadata: %v", err)
	}

	os.Remove(filepath.Join(baseDir, "client.db"))
	specials, _ := rpcClient.ListSpecialItems(context.Background())
	if len(specials) == 0 {
		t.Fatalf("No special blobs listed")
	}
	blobs, missing, err := rpcClient.DownloadItems(context.Background(), []string{specials[0].Hash})
	if err != nil {
		t.Fatalf("DownloadItems failed: %v", err)
	}
	if len(missing) > 0 {
		t.Fatalf("Missing blob: %s", missing[0])
	}
	if len(blobs) == 0 {
	...
	if len(actualData) < 64 {
		t.Fatalf("Downloaded payload too short for signature")
	}
	rcvSig := actualData[:64]
	rcvCipher := actualData[64:]
	if !crypto.VerifyRecoveryShard(key, rcvSig, rcvCipher) {
		t.Fatalf("Test payload signature verification failed")
	}

	dec, err := crypto.Decrypt(key, rcvCipher)
	if err != nil {
	...
	outF, _ := os.Create(filepath.Join(baseDir, "client.db"))
	io.Copy(outF, tr)
	outF.Close()
...
func TestServerDisasterRecovery(t *testing.T) {
	...
	engineA := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 2, 1, 20*1024*1024, true, nil, "", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, masterKey, "", "", 4, false, false, 0, 0)
	defer engineA.Wait()
	defer serverDB.Close()
...
func TestContactInfoPropagation(t *testing.T) {
	...
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 1, 1, 1024, true, nil, "127.0.0.1:8080", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", myContact, 4, false, false, 0, 0)
	defer engine.Wait()
...
func TestMultiPeerDeduplicationAndChallenge(t *testing.T) {
	...
	engine := server.NewEngine(serverDB, "", serverDBPath, serverBlobDir, serverQueueDir, 1, 1, 1024, true, nil, "127.0.0.1:8080", 1024, true, false, 8, 43200, 43200, 0.5, 720, 1440, 24, 4, -1, -1, -1, nil, "", "test@operator.com", 4, false, false, 0, 0)
	defer engine.Wait()
