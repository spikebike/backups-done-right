...
- **Swarm Distribution (Out-of-Band Streaming)**: An Outbound Worker actively looks for peers under their storage quota who don't already have a piece for the given shard. To support high-bandwidth (100mbit to 400gbit) connections without exhausting RAM, the system decouples the **Control Plane** from the **Data Plane**:
  - **Control Plane (Cap'n Proto)**: The sender negotiates what pieces are needed via lightweight RPCs (`offerShards` and `prepareUpload`), keeping multiplexed streams responsive.
  - **Data Plane (Raw libp2p `/bdr/data/1.0.0`)**: The sender initiates a dedicated raw byte stream. Data is piped natively from disk to socket using `io.CopyBuffer` in 4MB memory chunks. The receiver parses a small 41-byte header (`[OpCode][Checksum][Size]`) and streams the payload directly to a temporary file on disk using an `io.TeeReader` to generate the BLAKE3 hash on-the-fly. This guarantees the application never allocates a 256MB slice of memory, completely eliminating garbage collection spikes while fully saturating the network interface.
